        ...
        */
        ...
        buttonComputeTask5.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent event)
            {
                ...
                double[] solidMaterialLoads = new double[]{46800.0}; // from user
                double[] materialBurningTemperature = new double[]{13.8}; // depends on the material, from Class later
                double[] componentAvrSpeedBurn = new double[]{2.4}; // depends on the material, from Class later

                FireInspectionData data = FireInspectionData.create()
                        .setVolume(Double.parseDouble(textFieldForVolume.getText()))
                        .setApertureHeights(apertureHeights).setApertureSpaces(apertureSpaces)
                        .setHeight(Double.parseDouble(textFieldForHeight.getText()))
                        .setSolidMaterialsLoads(solidMaterialLoads);
                ...
                if (stat.getFireKind() == FireKind.VENTILATION_REGULATED) {
                    ...
                    textArea.append("Максимальна щільність теплового потоку з продуктами горіння, які йдуть через пройоми: " +
                        task6.computeMaxDensityOfHeatFlow(materialBurningTemperature, componentAvrSpeedBurn) + '\n'
                    );
                } else {
    ...
    private class TaskAction implements ActionListener {
        ...
        public void actionPerformed(ActionEvent event) {
            ...
            if (task.equals("task1")) {
                ...
                result1 = task2.computeMaxVolumeAverageTemperature(initialTemperature, materialBurningTemperature, componentAvrSpeedBurn);
                commentToResult1 = "Максимальна середньооб'ємна температура: ";

                result2 = task2.computeMaxTemperatureTime(materialBurningTemperature, componentAvrSpeedBurn);
                ...
                result2 = task3.computeMaxAverageTemperatureTime(materialBurningTemperature, componentAvrSpeedBurn);
                ...
                result1 = task4.computeMaxTemperature(materialBurningTemperature, componentAvrSpeedBurn);
                commentToResult1 = "Максимальна усереднина температура поверхні стін: ";

                result2 = task4.computeTimeAchievementMaxTemperature(materialBurningTemperature, componentAvrSpeedBurn);
                ...
                result1 = task5.computeMaxDensityForWallConstruction(materialBurningTemperature, componentAvrSpeedBurn);
                commentToResult1 = "Максимальна усереднина щільність ефективного потоку в конструкції стін: ";
                result1Auxiliary = task5.computeMaxDensityForCoverageConstruction(materialBurningTemperature, componentAvrSpeedBurn);
